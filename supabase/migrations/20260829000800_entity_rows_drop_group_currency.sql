-- Phase 2 / decision D1: entity-level rows stop carrying a group-currency amount.
--
-- Currency translation posts its own group-currency rows (D1). While the
-- reported rows ALSO carried amount_gc = amount_lc, every group total would
-- count the same balance twice - once untranslated, once translated.
--
-- Group currency is a property of a consolidation group, and an upload is
-- group-independent: at the moment a trial balance lands there is no single
-- correct group currency for it, and an entity belonging to two groups with
-- different currencies has two different right answers. So levels 00 and 01
-- carry amount_gc = 0 and translation supplies every group-currency figure,
-- including the same-currency case at rate 1. That makes translation a
-- mandatory step for every entity, which is how SAP Group Reporting behaves
-- and what the standard close template already sequences.
--
-- The demo seed is updated to match, and gains historical equity rates so
-- that translation produces a non-zero CTA worth looking at.
...
begin
  ...
  select b.tenant_id, e.id, a.id, m.id, p.id, b.version_id, b.fiscal_year, b.period,
         ...
         coalesce(s.amount_tc, 0), s.amount_lc, 0,
