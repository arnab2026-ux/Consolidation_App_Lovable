-- Phase 1 - Prompt 18 item 2: demo dataset.
--
-- seed_demo_dataset() builds a complete, internally consistent group so that
-- every calculation engine has something real to run against:
--
--   PARENT    USD   parent
--   SUB_US    USD   100%  PURCHASE        - same-currency path (no translation)
--   SUB_EU    EUR    80%  PURCHASE        - foreign currency + NCI
--   JV_SA     SAR    50%  PROPORTIONATE   - proportionate consolidation
--   ASSOC_IN  INR    30%  EQUITY          - equity method
--
-- Intercompany positions are chosen to yield one of every reconciliation
-- outcome: MATCHED (same currency and cross currency), DIFFERENCE (a
-- deliberate 50,000 USD mismatch) and ONE_SIDED.
--
-- Retained earnings is posted as the balancing figure, so every entity's
-- trial balance sums to exactly zero in local currency by construction.
--
-- Idempotent: with p_reset = true it clears the tenant's transactional and
-- master data first, so it can be re-run freely while building.

create or replace function public.seed_demo_dataset(
