-- Phase 2: close a silent translation gap.
--
-- "Net income for the period" is a balance sheet account translated at the
-- AVERAGE rate, so that it ties to the profit and loss result that produced
-- it. The seeded rule set had no rule for that combination: the balance
-- sheet rule wanted BS + CLOSING and the P&L rule wanted PL, so the account
-- was never translated. Its balance simply went missing from the translated
-- balance sheet and the CTA plug absorbed it - producing, for a USD entity
-- inside a USD group, a "cumulative translation adjustment" exactly equal to
-- net income where the correct answer is zero.
--
-- Two changes: a TR_NI rule for BS accounts carried at the average rate,
-- and coverage detection in the engine so an unclaimed account is reported
-- rather than quietly folded into the plug.
