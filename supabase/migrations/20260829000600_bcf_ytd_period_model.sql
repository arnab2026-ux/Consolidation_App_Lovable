-- Phase 2 prerequisite: make Balance Carry Forward respect the year-to-date
-- period model.
--
-- validate_upload_batch() requires each uploaded period's trial balance to
-- sum to zero, which only holds if the balance sheet and the P&L are on the
-- same basis - that is, both year-to-date as at the stated period. BCF was
-- summing prior-year source rows across every period with no period filter.
-- With one period per year that is correct by accident; with monthly data it
-- overstates the opening balance by a factor of twelve.
--
...
begin
  ...
  -- Balances are year-to-date (see docs/DECISIONS.md D3), so the prior year's
  -- closing position is the single highest period carrying data - NOT the sum
  -- of every period. Summing was harmless while each year held one period and
  -- a 12x overstatement the moment monthly data arrived.
  select max(f.period) into v_prior_close
    from fact_balances f
   where f.tenant_id = p_tenant and f.version_id = p_version
     and f.fiscal_year = p_year - 1
     and f.entity_id = p_entity and f.cons_group_id is null
     and f.posting_level in ('00', '01');
  if v_prior_close is null then
    return 0;
  end if;
