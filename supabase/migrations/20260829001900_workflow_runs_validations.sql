-- Phase 5: run the validation rule set from the close, and seed it.
--
-- The workflow's VALIDATION step ran one hardcoded trial-balance check. It now
-- runs every active validation rule for the entity and stores the findings on
-- task_run.log - a column the schema has always had and nothing used - so the
-- monitor can show exactly what failed rather than a bare status.
...
begin
  ...
  if p_reset then
    ...
    delete from validation_rule    where tenant_id = v_tenant;
  ...
  -- Default data-quality rules, so the close has something to validate
  -- against the moment it runs.
  perform seed_validation_rules();
