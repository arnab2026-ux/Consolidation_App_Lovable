-- Phase 5: the totals refresh cannot be concurrent.
--
-- REFRESH MATERIALIZED VIEW CONCURRENTLY is not allowed inside a transaction
-- block, and a plpgsql function is always inside one - as is every PostgREST
-- request. So the concurrent path could never actually be taken from either the
-- close or the API, and the Group Reports step failed with:
--
--   cannot refresh materialized view "public.mv_cons_totals" concurrently
--
-- Non-concurrent refresh takes an exclusive lock for its duration. That is
-- acceptable here: mv_cons_totals is only read by the statement rollup, it is
-- refreshed once at the end of a close, and it is small - one row per reporting
-- coordinate, not per posting.
--
-- The unique index stays. It costs nothing, and it is what a future
-- out-of-transaction refresh (a scheduled job, say) would need.

create or replace function public.refresh_cons_totals()
returns jsonb
language plpgsql
security definer
...
drop function if exists public.refresh_cons_totals(boolean);
